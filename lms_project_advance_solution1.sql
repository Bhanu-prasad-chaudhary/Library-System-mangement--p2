...
-- step ----issued_status == members == books == return_status
-- fliter books which is return
--- overdue > 30
SELECT 
...
LEFT JOIN -- left join here because we want above table(above join create a table ) with return_status only
...
--store procedure
CREATE OR REPLACE PROCEDURE add_return_records(p_return_id VARCHAR(10),p_issued_id VARCHAR(30),p_book_quality VARCHAR(15)) -- P_ all are enter by user
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    INSERT INTO return_status(return_id,issued_id,return_date,book_quality)
    VALUES
      (p_return_id,p_issued_id,CURRENT_DATE,p_book_quality); -- return_date is enter by system

     SELECT 
	    issued_book_isbn,
		issued_book_name
		INTO 
		 v_isbn, -- storing issued_book_isbn values in variable(v_isbn) to pass down
		 v_book_name
		FROM issued_status
		WHERE issued_id = p_issued_id; -- this give the isbn in update operation

	  UPDATE books
	  SET status ='yes'
	  WHERE isbn =v_isbn;

	  RAISE NOTICE 'Thank you for returing the book: %', v_book_name;-- this is similar like in python 
